...
void proximity(KdTree* tree, const std::vector<std::vector<float>>& points, int idx, std::vector<int> &cluster, std::vector<bool>& wasVisited, float distanceTol)
{
    wasVisited[idx] = true;
    ...
    for(int index : nearby) {
        if (!wasVisited[index]) {
            proximity(tree, points, index, cluster, wasVisited, distanceTol);
        }
    }
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
    ...
    std::vector<bool> wasVisited(points.size(), false);

    for (int i = 0; i<points.size(); ++i) {
        if (!wasVisited[i]) {
            std::vector<int> current_cluster {};
            proximity(tree, points, i, current_cluster, wasVisited, distanceTol);
